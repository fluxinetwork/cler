<?php
/*
Template Name: Gérerr adhèrentt
*/
?>
<?php get_header(); ?>
<article>
		...
		if( !empty($_GET['act'])):
			...
			if( $type_form == 'add' && current_user_can( 'publish_posts' ) && !is_adherent_cler() && get_adherent_status() == 'non_adherent' ):
				...
				$fields_annees_cotisation ='
					<option disabled selected value=""> - Quelle année ?</option>
					<option value="'.$l_year.'">Année de cotisation '.$l_year.'</option>
					<option value="'.$c_year.'">Année de cotisation '.$c_year.'</option>
					<option value="'.$n_year.'">Année de cotisation '.$n_year.'</option>';				
